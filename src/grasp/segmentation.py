...
class Segmentation(LeafSystem):
    def __init__(self, plant, bin_instance, camera_body_indices):
        ...
        self._camera_ports: List[CameraPorts]
        self._camera_ports = []
        for i in range(self._num_cameras):
            point_cloud_port = f"camera{i}_point_cloud"
            label_image_port = f"camera{i}_label_image"
            a = self.DeclareAbstractInputPort(
                point_cloud_port, model_point_cloud).get_index()
            b = self.DeclareAbstractInputPort(
                label_image_port, label_images).get_index()
            self._camera_ports.append(CameraPorts(a, b))
